...
void c18_analyze(Image const *src, int x1, int x2, int y1, int y2, C18Info *info)
{
	...
	for(int ky=y1+2;ky<y2;++ky)//analysis loop
	{
		...
		{
			const int
				*NNWW1	=src->data+4*(src->iw*(ky-2LL)+kx-2LL),
				*NNW1	=src->data+4*(src->iw*(ky-2LL)+kx-1LL),
				*NN1	=src->data+4*(src->iw*(ky-2LL)+kx+0LL),
				*NNE1	=src->data+4*(src->iw*(ky-2LL)+kx+1LL),
				*NNEE1	=src->data+4*(src->iw*(ky-2LL)+kx+2LL),
				*NWW1	=src->data+4*(src->iw*(ky-1LL)+kx-2LL),
				*NW1	=src->data+4*(src->iw*(ky-1LL)+kx-1LL),
				*N1	=src->data+4*(src->iw*(ky-1LL)+kx+0LL),
				*NE1	=src->data+4*(src->iw*(ky-1LL)+kx+1LL),
				*NEE1	=src->data+4*(src->iw*(ky-1LL)+kx+2LL),
				*WW1	=src->data+4*(src->iw*(ky+0LL)+kx-2LL),
				*W1	=src->data+4*(src->iw*(ky+0LL)+kx-1LL),
				*target	=src->data+4*(src->iw*(ky+0LL)+kx+0LL),
				offset1	[]={0, 0, 0};
#define DECL_NB(NB) {NB##1[0]-NB##1[1], NB##1[1]-NB##1[2], NB##1[2]-NB##1[0]}
			int
				NNWW2	[]=DECL_NB(NNWW),
				NNW2	[]=DECL_NB(NNW),
				NN2	[]=DECL_NB(NN),
				...
				WW2	[]=DECL_NB(WW),
				W2	[]=DECL_NB(W),
				offset2	[]={target[1], target[2], target[0]};
#undef  DECL_NB
#define DECL_NB(NB) {NB##1[0]-NB##1[2], NB##1[1]-NB##1[0], NB##1[2]-NB##1[1]}
			int
				NNWW3	[]=DECL_NB(NNWW),
				NNW3	[]=DECL_NB(NNW),
				NN3	[]=DECL_NB(NN),
				...
				WW3	[]=DECL_NB(WW),
				W3	[]=DECL_NB(W),
				offset3	[]={target[2], target[0], target[1]};
#undef  DECL_NB
#define DECL_NB(NB) {NB##1[0]-(NB##1[1]+NB##1[2])/2, NB##1[1]-(NB##1[2]+NB##1[0])/2, NB##1[2]-(NB##1[0]+NB##1[1])/2}
			int
				NNWW4	[]=DECL_NB(NNWW),
				NNW4	[]=DECL_NB(NNW),
				NN4	[]=DECL_NB(NN),
				...
				WW4	[]=DECL_NB(WW),
				W4	[]=DECL_NB(W),
				offset4	[]={(target[1]+target[2])/2, (target[2]+target[0])/2, (target[0]+target[1])/2};
#undef  DECL_NB
			int vmin[3], vmax[3];
