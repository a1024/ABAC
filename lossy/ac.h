...
//adaptive binary arithmetic coder (32 bit)
...
//arithmetic coder (64 bit)
#if 1
typedef struct AC64Struct
{
	unsigned long long lo, hi, cache;
	int nbits;

	DList *list;//encode-specific

	unsigned long long code;//decode-specific
	const unsigned char *srcptr, *srcend;
} AC64;
inline void ac64_enc_init(AC64 *ac, DList *list)
{
	ac->lo=0;
	ac->hi=0xFFFFFFFF;
	ac->cache=0;
...
inline void ac64_enc(AC64 *ac, int *CDF2, int nlevels, int sym)
{
	unsigned long long mingap, lo2, hi2, temp;
	int cdf_curr, cdf_next;

	for(;;)//renorm
	{
		mingap=(ac->hi-ac->lo)/CDF2[nlevels];
		if(ac->lo<ac->hi&&mingap>0)
			break;
		if(ac->nbits>=32)
		{
			dlist_push_back(ac->list, &ac->cache, 4);
			ac->cache=0;
			ac->nbits=0;
		}
		ac->cache|=(ac->lo&0x80000000)>>ac->nbits;//cache is written MSB -> LSB
		++ac->nbits;

		ac->lo<<=1;//shift out MSB
		ac->hi<<=1;
		ac->hi|=1;
	}
	...
	lo2=_umul128(ac->hi-ac->lo, cdf_curr, &temp);
	lo2=_udiv128(temp, lo2, CDF2[nlevels], &temp);
	
	hi2=_umul128(ac->hi-ac->lo, cdf_next, &temp);
	hi2=_udiv128(temp, hi2, CDF2[nlevels], &temp);

	//lo2=ac->lo+((unsigned long long)(ac->hi-ac->lo)*cdf_curr/CDF2[nlevels]);
	//hi2=ac->lo+((unsigned long long)(ac->hi-ac->lo)*cdf_next/CDF2[nlevels]);

	//acval_enc(sym, cdf_curr, cdf_next, ac->lo, ac->hi, lo2, hi2, ac->cache, ac->nbits);//

	ac->lo=lo2;
	ac->hi=hi2-1;//OBLIGATORY range leak guard
}
inline void ac64_enc_flush(AC64 *ac)
{
	int k2=0;
	do//flush
	{
...
inline void ac64_dec_init(AC64 *ac, const unsigned char *start, unsigned const char *end)
{
	ac->lo=0;
	ac->hi=0xFFFFFFFF;
	ac->nbits=32;
	...
	if(ac->srcend-ac->srcptr<4)
		LOG_ERROR2("buffer overflow");
	memcpy(&ac->code, ac->srcptr, 4);
	ac->srcptr+=4;

	if(ac->srcend-ac->srcptr<4)
		LOG_ERROR2("buffer overflow");
	memcpy(&ac->cache, ac->srcptr, 4);
	ac->srcptr+=4;
}
inline int ac64_dec(AC64 *ac, int *CDF2, int nlevels)
{
	...
	for(;;)//renorm
	{
		...
		--ac->nbits;
		ac->code<<=1;//shift out MSB		cache is read MSB -> LSB
		ac->code|=(unsigned)(ac->cache>>ac->nbits&1);
	...
	unsigned long long lo2, hi2, temp;
	int sym=0;
	...
	{
		sym=(L+R)>>1;
		code2=ac->lo+((unsigned long long)(ac->hi-ac->lo)*CDF2[sym]/CDF2[nlevels]);
		if(code2<ac->code)
			L=sym+1;
		else if(code2>ac->code)
			R=sym-1;
		else
		{
			found=1;
	...
	if(found)
		for(;sym<256-1&&ac->lo+((unsigned long long)(ac->hi-ac->lo)*CDF2[sym+1]/CDF2[nlevels])==ac->code;++sym);
	else
		sym=L+(L<256&&ac->lo+((unsigned long long)(ac->hi-ac->lo)*CDF2[sym+1]/CDF2[nlevels])<ac->code)-(L!=0);

	//buf[(iw*ky+kx2)<<2|kc]=(unsigned char)sym;

	unsigned cdf_start=CDF2[sym], cdf_end=CDF2[sym+1];

	lo2=_umul128(ac->hi-ac->lo, cdf_start, &temp);
	lo2=_udiv128(temp, lo2, CDF2[nlevels], &temp);
	
	hi2=_umul128(ac->hi-ac->lo, cdf_end, &temp);
	hi2=_udiv128(temp, hi2, CDF2[nlevels], &temp);
					
	//lo2=ac->lo+((unsigned long long)(ac->hi-ac->lo)*cdf_start/CDF2[nlevels]);
	//hi2=ac->lo+((unsigned long long)(ac->hi-ac->lo)*cdf_end  /CDF2[nlevels]);

	//acval_dec(sym, cdf_start, cdf_end, ac->lo, ac->hi, lo2, hi2, ac->cache, ac->nbits, ac->code);//
	ac->lo=lo2;
	ac->hi=hi2-1;//OBLIGATORY range leak guard
	return sym;
}
...
//arithmetic coder (32 bit)
#if 1
typedef struct AC32Struct
{
	unsigned lo, hi, cache;
	int nbits;
	...
	unsigned code;//decode-specific
	const unsigned char *srcptr, *srcend;
} AC32;
inline void ac32_enc_init(AC32 *ac, DList *list)
{
	ac->lo=0;
	ac->hi=0xFFFFFFFF;
	ac->cache=0;
...
inline void ac32_enc(AC32 *ac, int *CDF2, int nlevels, int sym)
{
...
inline void ac32_enc_flush(AC32 *ac)
{
	int k2=0;
	do//flush
	{
...
inline void ac32_dec_init(AC32 *ac, const unsigned char *start, unsigned const char *end)
{
	ac->lo=0;
	ac->hi=0xFFFFFFFF;
	ac->nbits=32;
...
inline int ac32_dec(AC32 *ac, int *CDF2, int nlevels)
{
	unsigned mingap;
	for(;;)//renorm
	{
