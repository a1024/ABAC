...
//	#define UNROLL_DECODER//do not unroll		//binary incompatible
	#define DISABLE_WGRAD//do not comment this


//THIC CODEC WAS MODIFIED FOR  Image struct with int[4]
#include"ac.h"
#ifdef ENABLE_GUIDE
...
//	};
	...
	if(args->fwd)
	{
		...
		int depth=image->depth[0], nlevels=1<<depth, half=nlevels>>1, mask=nlevels-1;
		int poolNch=0;
		...
		for(int ky=args->y1, idx=image->iw*args->y1*pixelstride;ky<args->y2;++ky)
		{
			ALIGN(16) int *rows[]=
			{
			...
			for(int kx=0;kx<image->iw;++kx, idx+=pixelstride)
			{
				int
#ifndef DISABLE_WGRAD
					*NN	=rows[2]+0*8,
...
#undef  PRED
		...
		{
			int *curr_hist=args->hist+((size_t)kc<<depth);
...
#undef  PRED
	...
	else
	{
		...
		int depth=image->depth[0], nlevels=1<<depth, half=nlevels>>1;
...
int f23_codec(Image const *src, ArrayHandle *data, const unsigned char *cbuf, size_t clen, Image *dst, int loud)
{
	...
	for(int k=0;k<nthreads;++k)
	{
		...
		arg->histsize=sizeof(int[MAXPREDS])<<image->depth[0];
	...
	if(fwd)
	{
		...
		if(loud)
		{
			ptrdiff_t
				...
				usize=((ptrdiff_t)image->iw*image->ih*image->nch*image->depth[0]+7)>>3;
			t0=time_sec()-t0;
	...
	else
	{
		...
		if(loud)
		{
			ptrdiff_t usize=((ptrdiff_t)image->iw*image->ih*image->nch*image->depth[0]+7)>>3;
			t0=time_sec()-t0;
