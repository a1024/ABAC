...
int quantize_ctx(int val)
{
	int negmask=-(val<0);
	val=abs(val);
	val=FLOOR_LOG2(val)+1;//[0~0x10000] -> [0~0x10]
	val=FLOOR_LOG2(val)+1;//[0~0x10] -> [0~4]
	val^=negmask;
	val-=negmask;
	//val>>=1;
	val+=CLEVELS>>1;
			...
			for(int kc=0;kc<nch;++kc)
			{
				...
				if(fwd)
				{
					...
					if(nbits)
					{
						while(nbits>8)
						{
							ac_enc_bypass(&ec, bypass>>(nbits-8)&0xFF, 8);
							nbits-=8;
						}
						ac_enc_bypass(&ec, bypass&((1<<nbits)-1), nbits);
					}
				}
				else
				{
					...
					if(delta>=(1<<CONFIG_EXP))
					{
						...
						{
							n-=8;
							bypass|=ac_dec_bypass(&ec, 8)<<n;
						}
						bypass|=ac_dec_bypass(&ec, n);
						delta=1;
						delta<<=CONFIG_MSB;
						delta|=msb;
						delta<<=nbits;
