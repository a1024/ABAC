...
#if NBITS==8
ALIGN(32) static const unsigned gramp[]=
{
...
static void update_CDF(int sym, unsigned *CDF)
{
...
#elif NBITS==8
	__m256i mamp=_mm256_set1_epi32(0x10000-(1<<NBITS));
	...
	for(int k=0;k<32;k+=2)
	{
		...
		update0=_mm256_srai_epi32(update0, 8);
		update1=_mm256_srai_epi32(update1, 8);
...
#elif NBITS==1
	CDF[1]+=(int)(((0x10000-(1<<NBITS))&-(1>sym))+1-CDF[1])>>8;
#endif
...
#undef  PRED
	...
	for(int k=0, p=1;k<depth;k+=NBITS, p<<=NBITS)
		nnodes+=p;
	size_t statssize=sizeof(int[NCTX<<NBITS])*nnodes*ctxsize*image->nch;
	...
	for(int ks=0;ks<(1<<NBITS);++ks)
		stats[ks]=ks<<(16-NBITS);
	memfill(stats+(1LL<<NBITS), stats, sizeof(int[1LL<<NBITS])*(nnodes-1LL), sizeof(int[1LL<<NBITS]));
	for(int ks=0;ks<nlevels;++ks)
	{
		int tidx=0;
		for(int kb=depth-NBITS;kb>=0;kb-=NBITS)
		{
			...
			unsigned *CDF=stats+((size_t)tidx<<NBITS);
			update_CDF(sym, CDF);
			tidx=(tidx<<NBITS)+sym+1;
		}
	}
	memfill(stats+((size_t)nnodes<<NBITS), stats, statssize-sizeof(int[1LL<<NBITS])*nnodes, sizeof(int[1LL<<NBITS])*nnodes);
			...
			for(int kc0=0;kc0<3;++kc0)
			{
				...
				if(fwd)
				{
					...
					for(int kb=depth-NBITS;kb>=0;kb-=NBITS)
					{
						...
						cdf=(CDF1[sym]+CDF2[sym]+CDF3[sym]+CDF4[sym])>>2;
						freq=(sym>=(1<<NBITS)-1?0x10000:((CDF1[sym+1]+CDF2[sym+1]+CDF3[sym+1]+CDF4[sym+1])>>2))-cdf;
				...
				else
				{
					...
					for(int kb=depth-NBITS;kb>=0;kb-=NBITS)
					{
						...
						for(int k=0;k<32;k+=2)
						{
							...
							mcdfA0=_mm256_add_epi32(mcdfA0, mcdfC0);
							mcdfA1=_mm256_add_epi32(mcdfA1, mcdfC1);
							mcdfA0=_mm256_add_epi32(mcdfA0, mcdfD0);
							mcdfA1=_mm256_add_epi32(mcdfA1, mcdfD1);
						...
						__m256i mcdfC0=_mm256_load_si256((__m256i*)CDF3+0);
						__m256i mcdfC1=_mm256_load_si256((__m256i*)CDF3+1);
						__m256i mcdfD0=_mm256_load_si256((__m256i*)CDF4+0);
						__m256i mcdfD1=_mm256_load_si256((__m256i*)CDF4+1);
						mcdfA0=_mm256_add_epi32(mcdfA0, mcdfB0);
						mcdfA1=_mm256_add_epi32(mcdfA1, mcdfB1);
						mcdfA0=_mm256_add_epi32(mcdfA0, mcdfC0);
						mcdfA1=_mm256_add_epi32(mcdfA1, mcdfC1);
